...
for epoch in range(epochs):
    ...
    for data in trainloader:
        inputs, _ = data
        # Add Gaussian noise to the inputs to make it a denoising autoencoder
        noisy_inputs = inputs + torch.randn_like(inputs) * 0.2
        ...
        latent, outputs = model(noisy_inputs)
        loss = criterion(outputs, inputs)  # Compare reconstructed images to original clean images
        loss.backward()
        optimizer.step()
...
for i in range(10):
    ...
    axes[0, i].axis('off')
    # Noisy images
    axes[1, i].imshow(noisy_inputs[i].cpu().permute(1, 2, 0))
    axes[1, i].axis('off')
    axes[0, i].axis('off')
